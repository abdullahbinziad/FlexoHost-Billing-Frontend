...
export interface DomainRenewalJobItem {
  _id: string;
  serviceId: string;
  domainDetailsId: string;
  invoiceId: string;
  clientId: string;
  domainName: string;
  registrar?: string;
...
export interface DomainRenewalJobsResponse {
  count: number;
  data: DomainRenewalJobItem[];
}
...
export const servicesApi = api.injectEndpoints({
  ...
  endpoints: (builder) => ({
    ...
    adminSuspendService: builder.mutation<
      { data: unknown },
      string | { serviceId: string; clientId?: string; sendNotification?: boolean }
    >({
      query: (arg) => ({
        url: `/services/admin/${typeof arg === "string" ? arg : arg.serviceId}/suspend`,
        method: "POST",
        body: typeof arg === "string" ? {} : { sendNotification: arg.sendNotification },
      }),
    ...
    adminChangePackage: builder.mutation<
      { data: unknown },
      { serviceId: string; plan: string; clientId?: string; sendNotification?: boolean }
    >({
      query: ({ serviceId, plan, sendNotification }) => ({
        url: `/services/admin/${serviceId}/change-package`,
        method: "POST",
        body: { plan, sendNotification },
      }),
      invalidatesTags: (_result, _err, { serviceId, clientId }) => {
        const tags: Array<{ type: "Service" | "Domain"; id: string }> = [
    ...
    adminChangePassword: builder.mutation<
      { data: unknown },
      { serviceId: string; password: string; username?: string; clientId?: string; sendNotification?: boolean }
    >({
      query: ({ serviceId, password, username, sendNotification }) => ({
        url: `/services/admin/${serviceId}/change-password`,
        method: "POST",
        body: { password, username, sendNotification },
      }),
      invalidatesTags: (_result, _err, { serviceId, clientId }) => {
        const tags: Array<{ type: "Service" | "Domain"; id: string }> = [
          { type: "Service", id: serviceId },
    ...
    getDomainRenewalJobs: builder.query<DomainRenewalJobsResponse, { status?: string; limit?: number; serviceId?: string; domainName?: string } | void>({
      query: (params) => ({
        url: "/services/admin/jobs/domain-renewals",
        params: params ?? {},
      }),
      transformResponse: (response: ApiResponse<DomainRenewalJobsResponse>) => response.data ?? { count: 0, data: [] },
      providesTags: [{ type: "Automation", id: "DOMAIN_RENEWAL_JOBS" }],
    }),
    retryDomainRenewalJob: builder.mutation<{ message?: string }, { jobId: string }>({
      query: ({ jobId }) => ({
        url: `/services/admin/jobs/domain-renewals/${jobId}/retry`,
        method: "POST",
      }),
      transformResponse: (response: ApiResponse<unknown>) => ({ message: response.message }),
      invalidatesTags: [{ type: "Automation", id: "DOMAIN_RENEWAL_JOBS" }, { type: "Automation", id: "SUMMARY" }],
    }),
    markDomainRenewalJobRenewed: builder.mutation<
      { message?: string },
      { jobId: string; expiresAt: string; note?: string; registrarTransactionId?: string }
    >({
      query: ({ jobId, ...body }) => ({
        url: `/services/admin/jobs/domain-renewals/${jobId}/mark-renewed`,
        method: "POST",
        body,
      }),
      transformResponse: (response: ApiResponse<unknown>) => ({ message: response.message }),
    ...
    syncDomainRenewalJobFromRegistrar: builder.mutation<{ message?: string }, { jobId: string }>({
      query: ({ jobId }) => ({
        url: `/services/admin/jobs/domain-renewals/${jobId}/sync-registrar`,
        method: "POST",
      }),
      transformResponse: (response: ApiResponse<unknown>) => ({ message: response.message }),
      invalidatesTags: [{ type: "Automation", id: "DOMAIN_RENEWAL_JOBS" }, { type: "Automation", id: "SUMMARY" }],
