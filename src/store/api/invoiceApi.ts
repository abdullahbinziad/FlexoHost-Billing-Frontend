...
export type InvoiceBulkAction = "mark_paid" | "mark_unpaid" | "mark_cancelled" | "send_reminder";
...
export const invoiceApi = api.injectEndpoints({
    endpoints: (builder) => ({
        ...
        bulkInvoiceAction: builder.mutation<
            InvoiceBulkActionResult,
            {
                invoiceIds: string[];
                action: InvoiceBulkAction;
                paymentMethod?: string;
        ...
        >({
            ...
            transformResponse: (response: any) => response.data,
            invalidatesTags: ["Invoice", "Service", "Domain", "Transaction"],
        }),
    }),
});
