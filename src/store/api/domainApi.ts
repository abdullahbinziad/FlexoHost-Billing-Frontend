...
export interface AdoptExistingRegistrarDomainRequest {
  clientId: string;
  domainName: string;
  registrar?: string;
  billingCycle: string;
...
export interface AdoptExistingRegistrarDomainResponse {
  serviceId: string;
  orderId?: string;
  orderItemId?: string;
  domainName: string;
...
export interface DomainLifecycleStatusOption {
  value: string;
  label: string;
}
...
export const domainApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getAdminDomainStatusOptions: builder.query<{ lifecycleStatuses: DomainLifecycleStatusOption[] }, void>({
      query: () => "/domains/admin/statuses",
      transformResponse: (response: ApiResponse<{ lifecycleStatuses: DomainLifecycleStatusOption[] }>) =>
        response.data ?? { lifecycleStatuses: [] },
      providesTags: [{ type: "Domain", id: "STATUS-OPTIONS" }],
    }),
    updateAdminDomainLifecycleStatus: builder.mutation<
      unknown,
      { serviceId: string; clientId?: string; lifecycleStatus: string; reason?: string }
    >({
      query: ({ serviceId, lifecycleStatus, reason }) => ({
        url: `/domains/admin/${serviceId}/status`,
        method: "PATCH",
        body: { lifecycleStatus, reason },
      }),
      invalidatesTags: (_result, _err, { serviceId, clientId }) => {
        const tags: Array<{ type: "Domain" | "Service"; id: string }> = [
          { type: "Domain", id: "ADMIN-INVENTORY" },
          { type: "Domain", id: serviceId },
    ...
    adoptExistingRegistrarDomain: builder.mutation<AdoptExistingRegistrarDomainResponse, AdoptExistingRegistrarDomainRequest>({
      query: (body) => ({
        url: "/domains/admin/adoptions",
        method: "POST",
        body,
      }),
      transformResponse: (response: ApiResponse<AdoptExistingRegistrarDomainResponse>) =>
        response.data as AdoptExistingRegistrarDomainResponse,
      invalidatesTags: (_result, _err, { clientId }) => [
        { type: "Domain", id: "ADMIN-INVENTORY" },
        { type: "Domain", id: `CLIENT-${clientId}` },
        { type: "Client", id: "LIST" },
      ],
    }),
    ...
    >({
      ...
      transformResponse: (response: ApiResponse<{ domain: string; status: string; expirationDate: string; nameservers: string[]; locked?: boolean; billing?: { serviceId?: string; status?: string; registrationDate?: string; nextDueDate?: string; billingCycle?: string; firstPaymentAmount?: number; recurringAmount?: number; currency?: string } }>) =>
    ...
    >({
      ...
      transformResponse: (response: ApiResponse<{ domain: string; status: string; expirationDate: string; nameservers: string[]; locked?: boolean; autoRenew?: boolean; registrar?: string; lifecycleStatus?: string; lifecycleReason?: string; syncStatus?: string; syncMessage?: string; lastRegistrarSyncAt?: string }>) =>
    ...
    renewDomain: builder.mutation<ApiResponse<RenewDomainResponse>, RenewDomainRequest>({
      query: (data) => ({
        url: `/domains/${encodeURIComponent(data.domainName)}/renew`,
        method: "POST",
        params: withClientId(data.clientId),
        body: data.years ? { duration: data.years } : {},
      }),
      invalidatesTags: ["Domain", "Invoice"],
    }),
...
export const {
  ...
  useAdoptExistingRegistrarDomainMutation,
