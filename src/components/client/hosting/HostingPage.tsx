...
export function HostingPage() {
  ...
  const { clientId, services, isLoading, isProfileLoading, noClient } = useClientServicesPage({
    type: "HOSTING",
    limit: 100,
  });
  ...
  const handleManage = (serviceId: string) => router.push(`/hosting/${serviceId}`);
  const handleOrderNew = () => { window.location.href = "/checkout"; };
  const handleRenew = () => { if (renewingSoon.count > 0) router.push("/checkout"); };
