...
interface OrderItemsCardProps {
    ...
    onRunModuleItem?: (item: any, allowUnpaidTrial?: boolean) => void;
    isRunningModule?: boolean;
}

export function OrderItemsCard({ order, manualConfigByItemId = {}, onManualConfigChange, onRunModuleItem, isRunningModule }: OrderItemsCardProps) {
                                            ...
                                            {showManualFallback && (
                                                <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                                                    ...
                                                    <div className="flex items-center space-x-2 md:col-span-3">
                                                        ...
                                                        />
                                                        <label htmlFor={`send-hosting-email-${itemId}`} className="text-xs font-medium text-gray-700 dark:text-gray-300">
                                                            Send New Hosting Account Email
                                                        </label>
                                                    </div>
                                                    {!isPaid && (
                                                        <div className="space-y-2 rounded-md border border-amber-200 bg-amber-50/60 p-3 md:col-span-3 dark:border-amber-900/60 dark:bg-amber-950/20">
                                                            <div className="flex items-center space-x-2">
                                                                <Checkbox
                                                                    id={`allow-trial-${itemId}`}
                                                                    checked={manualConfig.allowUnpaidTrial === true}
                                                                    onCheckedChange={(checked) => onManualConfigChange?.(itemId, { allowUnpaidTrial: checked === true })}
                                                                />
                                                                <label htmlFor={`allow-trial-${itemId}`} className="text-xs font-medium text-amber-800 dark:text-amber-200">
                                                                    Provision as unpaid trial
                                                                </label>
                                                            </div>
                                                            <div className="max-w-[180px] space-y-1">
                                                                <label className="text-[11px] font-medium text-amber-800 dark:text-amber-200">Trial days</label>
                                                                <Input
                                                                    type="number"
                                                                    min={1}
                                                                    max={60}
                                                                    value={manualConfig.trialDays || 7}
                                                                    onChange={(e) => onManualConfigChange?.(itemId, { trialDays: Number(e.target.value) || 7 })}
                                                                    disabled={manualConfig.allowUnpaidTrial !== true}
                                                                />
                                                            </div>
                                                        </div>
                                                    )}
                                                    <div className="md:col-span-3">
                                                        <Button
                                                            type="button"
                                                            size="sm"
                                                            disabled={isRunningModule || manualConfig.runModuleCreate === false || (!isPaid && manualConfig.allowUnpaidTrial !== true)}
                                                            onClick={() => onRunModuleItem?.(item, manualConfig.allowUnpaidTrial === true)}
                                                        >
                                                            {isFailed ? "Retry Module Create" : isPaid ? "Run Module Create" : "Run Trial Provision"}
                                                        </Button>
                                                    </div>
                                                </div>
                                            )}
                                                ...
                                                <>
                                                    ...
                                                    <div className="mt-4">
                                                        <Button
                                                            ...
                                                            size="sm"
                                                            variant="outline"
                                                            disabled={isRunningModule || manualConfig.runModuleCreate === false || !isPaid}
                                                            onClick={() => onRunModuleItem?.(item, false)}
                                                        >
                                                            {isFailed
                                                                ? `Retry ${String(item._raw?.actionType || "").toUpperCase() === "TRANSFER" ? "Transfer" : "Registration"} Module`
                                                                : `Run ${String(item._raw?.actionType || "").toUpperCase() === "TRANSFER" ? "Transfer" : "Registration"} Module`}
                                                        </Button>
                                                    </div>
                                                </>
                                            )}
