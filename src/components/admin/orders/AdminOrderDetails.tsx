...
export function AdminOrderDetails({ orderId }: AdminOrderDetailsProps) {
    ...
    const buildModuleItemPayload = (item: any, allowUnpaidTrial?: boolean) => {
        ...
        return {
            ...
            allowUnpaidTrial: allowUnpaidTrial === true || cfg.allowUnpaidTrial === true,
            trialDays: cfg.trialDays || 7,
        };
    };

    const handleRunModuleItem = async (item: any, allowUnpaidTrial?: boolean) => {
        try {
            const result = await runModuleCreate({
                orderId: order.id,
                items: [buildModuleItemPayload(item, allowUnpaidTrial)],
            }).unwrap();
            const failed = (result?.results || []).find((r: any) => !r.success);
            if (failed) {
                toast.error(failed.error || "Module action failed.");
                return;
            }
            await refetch();
            toast.success(allowUnpaidTrial ? "Trial provisioning started successfully." : "Module create completed.");
        } catch (e: any) {
            toast.error(e?.data?.message || e?.message || "Failed to run module create");
        }
    };
        ...
        try {
            ...
            if (manualCandidates.length > 0) {
                const moduleItems = manualCandidates.map((item: any) => {
                    const itemId = String(item._raw?._id || "");
                    const cfg = manualConfigByItemId[itemId] || {};
                    return buildModuleItemPayload(item);
                });
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 space-y-6">
                    <OrderItemsCard
                        ...
                        onRunModuleItem={handleRunModuleItem}
                        isRunningModule={isRunningModule}
                    />
                </div>
