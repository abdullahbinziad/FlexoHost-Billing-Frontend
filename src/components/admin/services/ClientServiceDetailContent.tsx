...
import { useSendClientEmailMutation } from "@/store/api/clientApi";
import {
  ServiceBillingCard,
  ServiceDetailHeader,
  ServiceDetailsCard,
...
export function ClientServiceDetailContent({
  ...
  const [emailTemplateKey, setEmailTemplateKey] = useState("custom");
  ...
  const openServiceEmail = () => {
    const identifier = service?.identifier && service.identifier !== "—" ? ` - ${service.identifier}` : "";
    setEmailTemplateKey("custom");
    setEmailSubject(`${service?.name || "Service"}${identifier}`);
    setEmailMessage("");
    setEmailOpen(true);
  };

  const handleSendServiceEmail = async () => {
    if (emailTemplateKey !== "custom") {
      try {
        ...
        toast.success("Service template email sent to client.");
        setEmailOpen(false);
      } catch (err: any) {
        toast.error(err?.data?.message || "Failed to send service template email.");
      }
      return;
    }
    ...
    try {
      ...
      toast.success("Service email sent to client.");
      setEmailOpen(false);
      setEmailMessage("");
    } catch (err: any) {
      toast.error(err?.data?.message || "Failed to send service email.");
    }
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <Card>
            <CardContent className="pt-6 space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-base font-semibold">Service Email</h3>
                  <p className="text-sm text-muted-foreground">
                    Send a manual email linked to this service.
                  </p>
                </div>
                <Button size="sm" variant="outline" onClick={openServiceEmail}>
      ...
      <Dialog open={emailOpen} onOpenChange={setEmailOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Send Service Email</DialogTitle>
            <DialogDescription>
              This message will be logged against this client and service.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Service</label>
              <Input value={`${service.name} · ${service.identifier}`} readOnly />
            </div>
            <div>
              ...
              <Select value={emailTemplateKey} onValueChange={setEmailTemplateKey}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="service.hosting_account_created">Hosting account created</SelectItem>
                  <SelectItem value="service.suspended">Service suspended</SelectItem>
                  <SelectItem value="service.unsuspended">Service restored</SelectItem>
                  <SelectItem value="service.terminated">Service terminated</SelectItem>
                </SelectContent>
              </Select>
            </div>
            {emailTemplateKey === "custom" && (
              <>
                <div>
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setEmailOpen(false)} disabled={isSendingEmail}>
              Cancel
            </Button>
            <Button type="button" onClick={handleSendServiceEmail} disabled={isSendingEmail}>
