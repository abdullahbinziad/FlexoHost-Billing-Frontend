...
export function ServiceModuleActionsCard({
  ...
  const actionArg = { serviceId: service.id, clientId, sendNotification };
  const changePackageArg = (plan: string) => ({ serviceId: service.id, plan, clientId, sendNotification });
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="pt-2 border-t space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
            <Button
              ...
              onClick={() => run(() => suspend(actionArg).unwrap())}
            >
            ...
            <Button
              ...
              onClick={() => run(() => unsuspend(actionArg).unwrap())}
            >
        ...
        <div className="flex items-start gap-2 rounded-md border bg-muted/30 px-3 py-2">
          <Checkbox
            id={`send-service-notification-${service.id}`}
            checked={sendNotification}
            onCheckedChange={(checked) => setSendNotification(checked === true)}
            className="mt-0.5"
          />
          <Label htmlFor={`send-service-notification-${service.id}`} className="text-sm leading-5 cursor-pointer">
            Send notification
            <span className="block text-xs font-normal text-muted-foreground">
              Send the matching client email and portal notification after a successful module action.
            </span>
          </Label>
        </div>
        ...
        <ConfirmActionDialog
          ...
          onConfirm={() => {
            run(() => terminate(actionArg).unwrap());
