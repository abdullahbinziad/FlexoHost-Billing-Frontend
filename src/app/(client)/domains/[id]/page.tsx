...
export default function DomainManage() {
  ...
  const domain = {
    ...
    autoRenewal: true,
    registrationDate: data.billing?.registrationDate ?? "",
    nameservers: data.nameservers ?? [],
    registrarLock: data.locked ?? true,
    sslStatus: "active" as const,
    billing: {
      firstPaymentAmount: data.billing?.firstPaymentAmount ?? 0,
      recurringAmount: data.billing?.recurringAmount ?? 0,
      billingCycle: data.billing?.billingCycle ?? "",
      paymentMethod: "",
      registrationDate: data.billing?.registrationDate ?? "",
      nextDueDate: data.billing?.nextDueDate ?? data.expirationDate ?? "",
